...
ordering of policies itself depends on candidate comparability (pending a sealed release).
...
| 6 | Symmetric-pipeline replication (§5.2, S7) | config `configs/symmetric_pipeline_dynamic_v1.json`; `python -m src.experiments.run_symmetric_pipeline_replication --run --confirmatory-authorized`; outputs `results/tables/symmetric_pipeline_dynamic_001/` (`CLAIM_INTERPRETATION.json`: Scenario A) |
| 7 | Size-matched zero-drift control (§5.3, §5.5, S8) | config `configs/size_matched_own_transformer_v1.json`; same driver with `--config`; outputs `results/tables/size_matched_own_transformer_001/` (`CLAIM_INTERPRETATION.json`: ATTENUATION); derived evidence–validation trade-off `results/tables/v1_22_1_editorial/` |
| 8 | Chronological replay evidence (§5.7, S2.6) | pre-enumerated matrix `results/tables/paper2_final_q1/chronological_replays.csv` (seeds 601–630); first six replays `results/tables/paper2_amendment_004/temporal.csv`, `results/tables/paper2_amendment_005/temporal_stratified.csv`, `results/tables/paper2_amendment_006/temporal_wed2thu.csv` |
