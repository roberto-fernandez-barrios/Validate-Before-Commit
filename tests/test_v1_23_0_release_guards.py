"""v1.23.0 release guards: the additive sealing of the two registered post-KBS blocks.
...
agree on 1.23.0. Hash verification is never weakened here; these tests only add pins.
"""
import hashlib
import json
import re
...
def test_historical_pins_unchanged_since_integration_commit():
    r = subprocess.run(["git", "show", f"{INTEGRATION_COMMIT}:results/tables/MANIFEST.sha256"],
                       cwd=REPO, capture_output=True, text=True)
    if r.returncode != 0:
        pytest.skip("integration commit not available in this checkout (shallow clone)")
    old = _pins(r.stdout)
    assert len(old) == 185
...
def test_verifier_reports_zero_unpinned_extras():
    ...
    assert "207 pinned CSVs match" in r.stdout and "(0 unpinned extras)" in r.stdout, r.stdout[-300:]


def test_version_surfaces_agree_on_1_23_0():
    ...
    assert 'version: "1.23.0"' in cff
    assert zen["version"] == "1.23.0"
    assert re.search(r"note\s*=\s*\{Version 1\.23\.0\}", bib)
    ...
    for name in ("main.tex", "main_ieee.tex"):
        ...
        assert "artifact version v1.23.0" in t, name
        assert "pending a subsequent sealed release" not in t, name
        assert "artifact version v1.22.0" in t, f"{name}: exact historical science DOI retained"


def test_ledger_and_final_manifest_register_b1_b2():
    ...
    assert m["artifact_version"] == "1.23.0"
