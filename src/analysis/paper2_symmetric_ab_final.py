"""final-kbs P0.1: the symmetric-A/B mechanism control, full condition set.
...
def one_dataset(name, ref, model_type="svc_rbf"):
    ...
    rows = {(c, k): [] for c in ("independent", "incumbent_fit", "challenger_fit",
                                 "own_transformer") for k in ("rand", "m2_minus_m1")}
    for seed in SEEDS:
        ...
        for cond in ("independent", "incumbent_fit", "challenger_fit", "own_transformer"):
            if cond == "own_transformer":
                ...
                fit_on = (XT if cond == "independent"
                          else ((X1 if inc_is_m1 else X2) if cond == "incumbent_fit"
