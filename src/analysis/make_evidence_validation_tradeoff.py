...
def main() -> None:
    ...
    caption = (
        ...
        "SVC-RBF, seeds 4001--4030). ``Cand.'' and ``Probe'' are nominal adjudicated "
        "labels per proposal (candidate training and gate probe); moving from 512 to "
        "2{,}000 samples per class adds $2\\times1{,}488 = 2{,}976$ nominal "
        "candidate-training labels per proposal, whereas the evaluated point/strict "
        "decision uses a 32-label probe. Nominal counts do not model inspected-flow or "
        "acquisition cost, so no economic dominance is claimed. BA contrasts are paired "
        "per seed (CI95, deterministic centered paired bootstrap); naive-2{,}000 vs "
        "never is the registered F1 contrast, gate gains at 2{,}000 are the registered "
        "F3 family (none Holm-significant), and gate-vs-never / 512-side cells are "
        "descriptive and uncorrected. The point-2{,}000 / strict-2{,}000 policies "
        "(supplementary block of the artifact CSV) differ from naive-2{,}000 by less "
        "than $0.14$ pp: all six registered gate contrasts at the matched size are "
        "non-significant and CI90-equivalent to naive within $\\pm$0.5 pp.")
