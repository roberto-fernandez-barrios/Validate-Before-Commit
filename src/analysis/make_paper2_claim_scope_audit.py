...
CLAIMS = [
    ...
    # --- v1.22 size-matched control (registered rewrite protocol,
    #     notes/size_matched_final_rewrite_protocol.md) ---------------------------------
    dict(claim_id="size_matched_equivalence",
         claim_text_short="size-matched challengers: naive_2000 == never (+-0.5, CI90, 3/3)",
         evidence_tier="registered confirmatory control (size-matched, seeds 4001-4030)",
         primary_source="results/tables/size_matched_own_transformer_001/equivalence.csv",
         unit_of_analysis="seed-level paired difference (30 fresh seeds)",
         multiplicity_family="size-matched F1 (Holm, 3)",
         operational_scope="zero drift, random proposal trigger, own-transformer, "
                           "balanced pools; not observed-data, not full drift",
         allowed=["equivalence margin"],
         forbidden=["harm persists with size-matched",
                    "no individual candidate can be worse"]),
    dict(claim_id="candidate_size_effect",
         claim_text_short="4x candidate-size asymmetry explains the residual zero-drift harm",
         evidence_tier="registered confirmatory control (nested candidate batches)",
         primary_source="results/tables/size_matched_own_transformer_001/paired_contrasts.csv",
         unit_of_analysis="seed-level paired difference (30 fresh seeds)",
         multiplicity_family="size-matched F2 (Holm, 3)",
         operational_scope="attributable in the evaluated setting; nested 512-in-2000 draws",
         allowed=["evaluated setting"],
         forbidden=["explains all harm everywhere"]),
    dict(claim_id="gate_value_conditional",
         claim_text_short="gates: no measurable value at matched size; conditional tool",
         evidence_tier="registered confirmatory control (F3/F4)",
         primary_source="results/tables/size_matched_own_transformer_001/multiplicity.csv",
         unit_of_analysis="seed-level paired difference (30 fresh seeds)",
         multiplicity_family="size-matched F3 (Holm, 6) + F4 secondary (Holm, 6)",
         operational_scope="matched-size zero-drift cells only; 512/frozen value unaffected",
         allowed=["no measurable"],
         forbidden=["gates remain useful at the matched size",
                    "candidate governance is universally required"]),
    dict(claim_id="attenuation_outcome",
         claim_text_short="registered outcome ATTENUATION; substantive mean neutralization",
         evidence_tier="frozen P/A/E rules, applied literally",
         primary_source=("results/tables/size_matched_own_transformer_001/"
                         "CLAIM_INTERPRETATION.json"),
         unit_of_analysis="machine-evaluable outcome rules (preregistered)",
         multiplicity_family="n/a (classification, not a test)",
         operational_scope="sign-based H5 criterion blocks formal ELIMINATION; mean H5 "
                           "future value ~0 at matched size; never retro-classified",
         allowed=["attenuation", "proposal-level variability"],
         forbidden=["formally elimination", "attenuation means the mean harm remains",
                    "50% probability of harm"]),
]
