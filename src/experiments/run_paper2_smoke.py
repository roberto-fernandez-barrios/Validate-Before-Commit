...
def make_windows(
    *,
    ...
    """
    Create reference/current windows for a first smoke test.

    no_drift:
        both windows come from the same distribution.

    mean_shift:
        current window is shifted in the first half of the features.

    gaussian_noise:
        current window has extra Gaussian noise.

    feature_dropout:
        current window has a fraction of features zeroed.
    """
...
def run(args: argparse.Namespace) -> Path:
        ...
        for dim in dims:
            for drift_type in drift_types:
                ...
                for severity in active_severities:
                    run_idx += 1
                    rng = np.random.default_rng(seed + 10_000 * run_idx)
                    ...
                    rows.append(
                        {
                            ...
                            "drift_detected": int(pred.drift_detected),
                            "gamma": pred.gamma,
    ...
    output_path = outdir / "paper2_smoke_mmd_rbf.csv"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Paper 2 smoke test: RBF-MMD drift detector with permutation calibration."
    )

    parser.add_argument(
        "--outdir",
        type=str,
        default="results/raw/smoke_001",
    )
    parser.add_argument(
        "--dims",
        type=str,
