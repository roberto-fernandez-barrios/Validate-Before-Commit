...
| Natural-prevalence probes: the initial "probe need not be balanced" claim is RETRACTED; the corrected Binomial rerun holds to π≈0.05 and dissolves at π=0.01 | `paper2_phase2j_probe_prevalence_001/` (superseded) + amendment-006 binomial arms; main §5.7, Supp §S2.3 |
| Historical frozen-transformer size/PCA controls (size-matching deepened full-drift harm under the frozen incumbent-owned representation; superseded by the registered full-drift size control under self-contained pipelines, row below) | `paper2_phase2k_size_dim_controls_001/`, Supp §S1.5; main §5.4 |
| Registered replication (harness v2, common streams, disjoint partitions, fresh seeds): all criteria pass; per-trigger mechanism (r_deg −0.65..−0.70 vs r_score ≈0) | `paper2_v2_replication_001/`, main §5.1 (historical frozen-policy diagnostic), Supp §S2.11, tag `harness-v2-protocol` |
...
| Chronological streams (corrected runner): deep-benefit recoveries on all CICIDS days; gate premium on Friday/Thursday BA, gate ahead on overall accuracy; no premium on Wednesday or external UNSW | `paper2_amendment_004/temporal.csv`, temporal-streams table (Supp §S2.6), main §5.7 |
| Pre-enumerated chronological matrix (seven further replays; net harm unobserved; healthy UNSW timelines vs the unresolved Wednesday counterexample) | `paper2_final_q1/chronological_replays.csv`, tab:chronological_q1, main §5.7 |
| Observed-data 64-flow matrix, VBC-SG frontier, prevalence sweep (final-kbs) | `paper2_final_kbs/`, `paper2_final_q1/budget_frontier.csv`, Tables causal_probe/symmetric_ab/budget_frontier (Supp §S2.12), main §5.7 |
| Size-matched self-contained challenger control (ATTENUATION; matched-size means compatible with ±0.5 pp under zero drift, PortScan boundary-close; candidate-size effect Holm-significant; no gate gain at 2,000/class) | `size_matched_own_transformer_001/` (CLAIM_INTERPRETATION.json), tab:size_matched, main §5.3, §5.5, Supp §S8 |
| Historical baseline blocks (full all-block matrix Supp §S2.12; label-free rules, ATC/DoC, DDM/ADWIN, update generators, McNemar/CS/VBC-SG; descriptive across blocks, paired within block; superseded by the registered common-harness comparison wherever the same method and question are evaluated under the final harness) | `paper2_policy_frontier_005/frontier.csv`, `paper2_phase2h_labelfree_gates_001/`, `paper2_phase2i_replay_baseline_001/`, `paper2_amendment_004/robustness.csv`, `paper2_amendment_008/summary.csv`, `paper2_final_q1/budget_frontier.csv`; Supp §S2.12 (tab:baselines_full) |
| Size-matched control under drift (B2; registered block, sealed in v1.23.0): naive₂₀₀₀−naive₅₁₂ = +0.82/+1.66/+1.00 BA points (PortScan/UNSW-Recon/ToN-IoT; Holm-sig.; HOMOGENEOUS-SIZE BENEFIT); always-deploy beneficial at both sizes; 0/6 positive gate effects at 2,000/class, strict UNSW-Recon a resolved cost (−0.34) | `post_kbs_size_matched_drift_001/` (CLAIM_INTERPRETATION.json), tab:size_matched_drift, main §5.4, Supp §S9; protocol `notes/post_kbs_size_matched_drift_protocol_001.md`, config `configs/post_kbs_size_matched_drift_v1.json` |
| Registered common-harness comparison (B1; registered block, sealed in v1.23.0): at 2,000/class parity ATC and the calibrated ensemble are COMPATIBLE with always-deploy at full drift, DoC/replay/river-DDM/river-ADWIN pay MATERIAL COSTS; no policy attains S1; S4 ordering change fires for ATC and the ensemble; 15/15 resolved method×size interactions negative; no adaptive-NIDS system reproduced end to end | `post_kbs_common_harness_baselines_001/` (cell_classification.csv, statements.csv, budget_table.csv), tab:common_harness, main §5.6, Supp §S10; protocol + amendment `notes/post_kbs_common_harness_baselines_{protocol,amendment}_001.md`, config `configs/post_kbs_common_harness_baselines_v2.json` |
| Final exact-feature-disjoint B2 sensitivity (v1.24.0): naive₂₀₀₀−naive₅₁₂ = +0.53/+1.67/+0.38 BA points; all three positive and Holm-resolved, material in PortScan/UNSW and sub-material in ToN; PARTIAL ROBUSTNESS | `ijis_exact_value_disjoint_b2_001/`, main §5.7, Supp §S11; protocol `notes/ijis_exact_value_disjoint_sensitivity_protocol_001.md`, config `configs/ijis_exact_value_disjoint_b2_v1.json` |
| Final exact-feature-disjoint B1 sensitivity (v1.24.0): policy conclusions PARTIALLY ROBUST (4/6 registered predicates); size-dependent ordering and no global dominance survive; ATC/calibrated-ensemble retention statements narrow | `ijis_exact_value_disjoint_b1_001/`, main §5.7, Supp §S11; config `configs/ijis_exact_value_disjoint_b1_v1.json` |
| Symmetric-pipeline replication (Scenario A): mean full-drift harm does not persist under self-contained pipelines (+7.21/+2.55/+1.03); ownership interaction up to +5.98; zero-drift harm persists (−1.74/−0.65 material, −0.38 resolved) and gates recover it (6/6 Holm-sig.); unsw_zero strict = recall↔FPR trade-off | `symmetric_pipeline_dynamic_001/` (CLAIM_INTERPRETATION.json), tab:symmetric_pipeline, main §5.2, Supp §S7 |
