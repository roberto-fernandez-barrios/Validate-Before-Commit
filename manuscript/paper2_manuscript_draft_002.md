...
Operational machine-learning intrusion detectors are periodically retrained to cope with concept drift, and the decision of *when* to retrain is usually delegated to a drift detector. We show this is the wrong control variable. Whether retraining helps is governed almost entirely by how degraded the deployed model already is — the benefit of adaptation correlates with no-adaptation accuracy at r ≈ −0.82 to −0.89 — a quantity drift detectors, which only measure distributional change, cannot observe. Consequently the detector is not the operative lever, and drift-triggered retraining ranges, across three public benchmarks (CICIDS2017, UNSW-NB15, ToN-IoT), from strongly beneficial to net-harmful: for a fragile classifier, not adapting outperforms every triggered strategy. We recast readaptation as a cost-aware operational decision and introduce a deployable, label-efficient *validate-before-commit* gate: a candidate is retrained on each trigger but committed only if it beats the incumbent on a small labeled probe. The gate needs only tens of labeled flows (as few as ~8 per confirmed drift), tolerates validation labels up to 20 windows stale, and *fails safe* under adversarial supervision — never harmful even with 40% of validation labels flipped. Under a pre-registered protocol with 30-seed 95% confidence intervals it generalizes across two drift detectors and four downstream classifiers: it converts harm to net benefit (significantly beating naive triggering and never-adapting) and preserves the gain where retraining helps. Two ablations delimit the method — simple confirmation/cooldown policies and a zero-label variant both fail. Results are reproducible from a public artifact.
...
This paper challenges that assumption on empirical grounds. Across three public benchmarks (CICIDS2017, UNSW-NB15, ToN-IoT) and multiple attack regimes we find the premise beneath the whole detection-centric framing is false: **drift-triggered retraining is not always beneficial.** Its value ranges from strongly positive to negative — for a fragile deployed model in a ToN-IoT scanning regime, *no adaptation* outperforms every triggered strategy we test. Whether retraining helps is governed not by how much the distribution moved but by how much the deployed model has already degraded: the two correlate at r ≈ −0.82 to −0.89 (with confidence intervals, across four downstream classifiers). A drift monitor, which measures distributional change, cannot observe this quantity, which is precisely why sensitive detectors — and simple confirmation/cooldown policies built on them — fail to prevent harmful retraining. This holds even when the detector is a quantum-kernel MMD monitor rather than a classical two-sample test: the detector family is not the lever.

We therefore argue that adaptive retraining should be recast from a *detection* problem to a cost-aware *decision*: not "did the distribution change?" but "will retraining improve the classifier?". This reframing yields a simple, deployable solution. We propose a **validate-before-commit** gate that, on each triggered drift, retrains a candidate model but deploys it only if it beats the incumbent on a small labeled probe drawn from current traffic. Across benefit, mixed and harmful regimes, the gate (pre-registered, 30 seeds) preserves the gains of retraining where it helps, avoids harm where it does not, and significantly outperforms both naive triggering and never adapting. It is cheap and robust: as few as ~8 labeled flows per confirmed drift suffice; it tolerates validation labels up to 20 windows stale; it *fails safe* under adversarial supervision (never harmful even with 40% of validation labels flipped); and it generalizes across two drift detectors and four downstream classifiers. A zero-label variant fails, showing that a few labels, spent on the decision rather than the detector, are both necessary and sufficient.
...
If the decision is the lever, the natural fix is a smarter trigger policy. We pre-registered a grid of alarm-confirmation (k-of-n) and cooldown policies and evaluated them against fixed success criteria (preserve benefit in PortScan; avoid harm in ToN-IoT; do not degrade UNSW). None passed: in the harmful regime the tested policies remained far below no-adaptation and did not reduce adaptations relative to a conservative legacy baseline (Table 5; Appendix E). Confirmation and cooldown act on the *persistence* of distributional change, which is exactly the signal that is uninformative about whether adaptation helps. This negative result motivates a policy that acts on the right quantity.
...
The gate passes every criterion for both detectors. In the harmful regime it turns net-harmful naive triggering into net benefit: ToN-IoT gain moves from −1.36 (KS-max) and −3.69 (QK-ZZ) points to +0.93 and +1.06, **significantly above naive triggering** (+2.30 points, CI95 [1.15, 3.63] for KS-max; +4.74, [2.47, 7.69] for QK-ZZ; Table 3) and, notably, **significantly above no-adaptation itself** (+0.93, [0.53, 1.36]; +1.06, [0.77, 1.40]) — the gate does not merely stop adapting, it commits the minority of beneficial adaptations while rejecting the harmful majority. In the benefit regime it preserves the full gain (PortScan +8.3/+8.4 points, statistically indistinguishable from naive), and it does not degrade the mixed regime. The label cost is small: roughly 100–160 labeled flows over a 100-window stream at budget 32.
...
*Label latency:* the probe need not be current. Drawing it from traffic labeled up to 20 windows in the past (Fig. 7) leaves the gate safe and effective at every lag — in the harmful regime the gate holds at +0.9 to +1.1 (vs naive −1.36) for lags of 5, 10 and 20 windows, and the benefit regimes are essentially unchanged — so the method tolerates realistic labeling delay rather than requiring instantaneous labels. *Harm breadth:* the net-harm of naive triggering (with the fragile SVC-RBF downstream) is not confined to one ToN-IoT regime — it also appears in ToN-IoT DDoS (naive **−16.81**) and Injection (−1.21), and the gate converts all three to net-positive (+1.26, +0.80, +0.93). *Cost knob:* raising the commit margin ε from 0 to 0.01–0.02 cuts committed adaptations by ~30% (e.g. PortScan 3.1→2.2 commits) at equal downstream gain, giving a simple operating characteristic that trades update frequency for benefit. *Adversarial validation labels:* because the gate trusts a labeled probe, we test an attacker (or faulty labeler) that flips a fraction of the probe labels (Fig. 8). The gate fails safe: even with 40% of validation labels corrupted — near the point where the probe is pure noise — it still avoids harm and still matches-or-beats naive triggering in every regime (ToN-IoT degrades gracefully from +0.93 to +0.32 but never goes negative; benefit and mixed regimes are essentially unchanged). Label noise perturbs the deployed and candidate estimates symmetrically, so it moves the gate along the naive-to-oracle line without pushing it below the safe baseline. *Significance of the safety claim.* Paired bootstrap CIs in the harm regime confirm the guarantee is not noise: the gate significantly outperforms naive triggering under every label-latency and every probe-poison level tested (CI95 excludes zero), and in no robustness condition — across downstream models, latencies or poison levels — is it significantly *worse* than either naive triggering or no-adaptation. Where the gate is not significantly better than naive (the robust downstream models, where naive is already non-harmful), it is statistically indistinguishable from it, i.e. never worse.
