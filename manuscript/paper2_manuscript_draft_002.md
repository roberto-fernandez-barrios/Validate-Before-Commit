...
### 5.6 Label efficiency and benefit-regime breadth (post-registration robustness)

Two extensions beyond the pre-registered design (explicitly labeled as robustness, not part of the confirmatory claim) strengthen the two title claims. First, sweeping the probe budget from 8 to 128 flows (KS-max, 30 seeds; Fig. 5) shows the gate is remarkably label-cheap: in the harmful regime even the smallest budget avoids harm — roughly 8 labeled flows per confirmed drift (≈23 labels per stream) already lift ToN-IoT gain from −1.36 (naive) to +0.38, rising to a plateau near +1.1 by budget 32–64; in the benefit regime the gain is flat and preserved at every budget. A few dozen labels, not hundreds, therefore suffice. Second, beyond PortScan the gate preserves — and slightly improves — the benefit of adaptation in the two strongest benefit regimes, Wednesday (naive +15.39 → gate +16.32) and DDoS (naive +24.81 → gate +25.27), so it never sacrifices benefit where adaptation helps, across three benefit regimes.
...
- **Fig. 1** regime spectrum · **Fig. 2** mechanism law (r=−0.89) · **Fig. 3** oracle-regret vs harm-frequency · **Fig. 4** the gate (both detectors) · **Fig. 5** label-efficiency frontier (probe budget 8–128).
